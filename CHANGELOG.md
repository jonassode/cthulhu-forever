...
0.10.2 - Update app version to 0.10.1  
0.8.9 - Update stored character delete button styling; Remove My Characters header box; Remove 'Your saved' from character description; Add import button to My Characters tab; Change import condition to exclude active character; Validate identity block in character data; Fix imported character local storage persistence; Fix tab hover border rendering; Add local storage reassurance to import errors; Persist editable character values to local storage; Refine character library card actions; Add local storage character library  
0.8.8 - tweak: simplify Stone Age lifestyle copy and layout; fix: move Stone Age lifestyle selection to step 2; Fix implementation status formatting in bug analysis; fix: enforce Stone Age leader fixed community bond; Correct implementation status of Community Bonds; refine req-3 to rename-only change; fix stone age other tribe typed skill; Fix community bond rounding for REQ-1; docs: label Stone Age findings with req ids; docs: add Stone Age bug analysis spec; Update outcast clan display on both sheet exports; Plan outcast sheet display update; Fix Stone Age cast-out bond and bonus pick rules; fix stale hunter-gatherer clan bond when switching lifestyles; Restrict hunter-gatherer resource bonus actions; Fix hunter-gatherer clan bond handling in creator; Fix clan prosperity display for hunter-gatherer lifestyle; Move Stone Age metadata to second header row; Show clan details on Stone Age sheets; fix: require stone age hunter-gatherer clan details; Reset Stone Age lifestyle on start over; Fix Stone Age character import; Add lifestyle to Stone Age sheet views; Change font mapping for 'stone' to 'Londrina Outline'; Change font name from 'Shojumaru' to 'Rock 3D'; fix: use Shojumaru on Stone Age original sheet; Update Stone Age era description and date range; Potential fix for pull request finding; Potential fix for pull request finding; Potential fix for pull request finding; Potential fix for pull request finding; Replace STONE_AGE_SKILL_DESCRIPTIONS with official sourcebook text; Add STONE_AGE_SKILL_DESCRIPTIONS and wire into getSkillDescription; Move Stone Age above Apocthulhu in era timeline; Remove temporary PDF files; Fix code review issues: clanProsperity import, Warrior duplicate skill, resource sacrifice clarification; Add Stone Age era display, clan name, and font support; Fix syntax error in canProceed function; Add Stone Age skills, archetypes, and lifestyle selection  
