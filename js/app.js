...
function getBonusPointsTotal() {
  ...
  let total = 10;
  if (hasHunterGathererResources() && state.castOut) total += 2;
...
function toggleCastOut(checked) {
  state.castOut = checked;
  if (state.age === 'stone' && state.lifestyle === 'hunter_gatherer' && state.bonds.length > 0) {
    if (checked) {
      state.bonds[0] = createEmptyBond();
  ...
  const bondsHtml = `
    ...
    <div style="display:flex;flex-direction:column;gap:0.75rem;">
      ${state.bonds.map((b, i) => {
        ...
        const canSacrifice = isCommunity && !isFixedClanBond && effectiveResources > 0 && !state.resourcesSetToZero;
...
function toggleBondSetToOne(index) {
  ...
  if (bond.setToOne) {
    ...
    if (getEffectiveResources() <= 0 || state.resourcesSetToZero) return; // cannot sacrifice a bond that starts at 0
    bond.setToOne = true;
...
function exportToOriginalSheet() {
  ...
  const ERA_FONT_MAP = {
    ...
    stone:       { name: 'Londrina Outline',    url: 'Londrina+Outline' },
  };
...
/* statistics table */
.stats-block { border: 1.5px solid #555; align-self: start; }
...
.stats-table th:first-child { border-left: none; }
...
.cb-checked { }
.inc-adapted { background: #fff; color: #000; border: 1px solid #555; border-radius: 3px; font-family: Arial, Helvetica, sans-serif; font-size: 5pt; text-transform: uppercase; letter-spacing: .15em; padding: 1px 5px; margin-left: 4px; }
  ...
  <div class="perm-inc-row">
    ...
    <div class="inc-block">
      ...
      <div class="inc-line">
        ...
        ${(state.violenceChecked || [false,false,false]).every(c => c) ? '<span class="inc-adapted">Adapted</span>' : ''}
      </div>
      <div class="inc-line">
        ...
        ${(state.helplessnessChecked || [false,false,false]).every(c => c) ? '<span class="inc-adapted">Adapted</span>' : ''}
      </div>
      </div>
    </div>
  </div>
